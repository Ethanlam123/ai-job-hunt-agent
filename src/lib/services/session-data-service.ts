...
export function createSessionSummary(session: any): SessionSummary {
  const cvDocument = session.documents?.find((doc: any) => doc.documentType === 'cv' || doc.document_type === 'cv')
  const jdDocument = session.documents?.find((doc: any) => doc.documentType === 'jd' || doc.document_type === 'jd')

  // Determine session status
  let status: 'completed' | 'processing' | 'failed' = 'processing'
  if (session.completedAt || session.completed_at) {
    status = 'completed'
  ...
  const stageDisplay = getStageDisplayName(session.currentStage || session.current_stage)

  // Create result summary
  const summaryText = getResultSummary(session.currentStage || session.current_stage, session.resultCounts)

  return {
    id: session.id,
    analysisType: session.analysisType || session.analysis_type || 'general',
    stageDisplay,
...
function getResultSummary(stage: string | null, counts: SessionResultCounts): string {
